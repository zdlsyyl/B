...
parser.add_argument("--output_name", type=str, required=True, help="The name (or path) of the motion npz file.")
...
# Pre-defined configs
##
from whole_body_tracking.robots.urdf7 import MY_ROBOT_CFG
...
def _resolve_output_path(output_name: str) -> str:
    """Resolve output npz path and ensure directory exists."""
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, joint_names: list[str]):
    ...
    while simulation_app.is_running():
        ...
        # collect data until first loop completes
        if not file_saved:
        ...
        if reset_flag and not file_saved:
            ...
            # Save motion locally (FIX: keep it inside this block!)
